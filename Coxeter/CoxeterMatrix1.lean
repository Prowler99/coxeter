...
/-
Q: I do not understand this part.
-/
def of (x : α) : G := QuotientGroup.mk' N (FreeGroup.of x)


-- Q: The set of simple reflections
@[simp]
abbrev SimpleRefl := Set.range (of m)

/-
We denote by S the set of simple reflections
-/
local notation "S" => (SimpleRefl m)

/-
Let $∪_{s ∈ S} g s g ^{-1}$ be the set of reflections.
-/
@[simp]
abbrev Refl : Set G := Set.range <| fun ((g, s) : G × S) => g * s * g⁻¹

/-
We denote by T the set of reflections of G.
-/
...
/-
Lemma: Let t ∈ G, then t ∈ T if and only if
there is a simple reflection s ∈ S amd g ∈ G, such that g s g⁻¹ = t
-/
@[simp]
...
/-
Lemma: Let t ∈ T be a reflection and s ∈ S. Then s t s⁻¹ is also a reflection.
-/
@[simp]
...
/-
Lemma: Let t ∈ T be a reflection and g ∈ G. Then g t g⁻¹ is also a reflection.
-/
@[simp]
...
/-
Lemma: Let t ∈ T be a reflection. Then t^2 =1.
-/
...
/-
Lemma: Let t ∈ T be a reflection. Then t = t⁻¹.
-/
...
------------------------------------------------------------------Move this part to length
-- DLevel 1
/-
Q: why do we need m g, instead of just g?
-/
...
-- DLevel 1
/-
lemma: Let g ∈ G and s ∈ S, then we have ℓ(g) ≠ ℓ (s g).
-/
...
-- DLevel 1
/-
lemma: Let g ∈ G and s ∈ S, then we have ℓ(g) ≠ ℓ (g s).
-/
...
-- DLevel 1
/-
lemma: Let g ∈ G and s ∈ S, then we have either ℓ (g s) = ℓ(g) + 1
 or ℓ (g * s) = ℓ(g) - 1.
-/
...
-- DLevel 1
/-
Lemma: Let g ∈ G and s ∈ S be such that ℓ(s * g ) ≤ ℓ(g), then we have ℓ (s * g ) < ℓ (g).
-/
